...
const Chest = () => {
  ...
  const {setOpenChestEx} = useContext(Contexts)
  useEffect(() => {
    const fetchBackExercices = async () => {
      if (!moveToggle) {
        ...
        exercise.filter((ex) => ex.bodyPart === 'chest')
      );
      } else if (moveToggle) {
        const backData = await fetchExercicesData('https://exercisedb.p.rapidapi.com/exercises', exercicesOptions);
        setExercices(backData)
        setExercices((exercise) =>
        exercise.filter((ex) => ex.bodyPart === 'chest' && ex.equipment === 'body weight')
      );
      }
    }
    fetchBackExercices();
  ...
  return (
    <div className='ex-container'>
      <i onClick={() => setOpenChestEx(false)} class="fa-solid fa-circle-xmark"></i>
      <h1 className='exercices-header' style={{padding: '30px 0', textAlign: 'center'}}>Exercices of : Chest</h1>
