...
const UpperLegs = () => {
  ...
  const {setOpenUpperLegsEx} = useContext(Contexts)
  useEffect(() => {
    const fetchBackExercices = async () => {
      if (!moveToggle) {
        ...
        exercise.filter((ex) => ex.bodyPart === 'upper legs')
      );
      } else if (moveToggle) {
        const backData = await fetchExercicesData('https://exercisedb.p.rapidapi.com/exercises', exercicesOptions);
        setExercices(backData)
        setExercices((exercise) =>
        exercise.filter((ex) => ex.bodyPart === 'upper legs' && ex.equipment === 'body weight')
      );
      }
    }
    fetchBackExercices();
  ...
  return (
    <div className='ex-container'>
      <i onClick={() => setOpenUpperLegsEx(false)} class="fa-solid fa-circle-xmark"></i>
